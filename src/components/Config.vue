  ...
  <row :gutter="20">
    <el-col :span="12" :offset="6">
      <el-input v-model="name">
        <template slot="prepend"> Config Name </template>
      </el-input>
    </el-col>
    <el-col :span="12" :offset="6">
    ...
    <el-col :span="12" :offset="6">
      <el-input v-model="seconds" min=0 type="number">
        <template slot="prepend"> Seconds </template>
      </el-input>
    </el-col>
  </row>
  <row class="center buttonRow">

    <el-button type="primary" v-on:click="setConfig"> Make this My Config </el-button>
  ...
  methods: {
    ...
    setConfig: function(){
      ...
      if (min === 0 && sec === 0 && hours === 0) {
        ...
        this.$data.saveErrorMessage = "Come on! You gotta work for at least a second before going on break!"
        setTimeout(function(){
          ...
          this.$store.dispatch('setConfig', { name: this.$data.name, time: hours*60*60 + min*60 + sec});
          this.$data.minutes = 0;
          this.$data.seconds= 0;
          this.$data.hours = 0;
          this.$data.saveError= false;
