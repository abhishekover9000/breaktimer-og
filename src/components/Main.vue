...
<div>
  <div class="sideContainer">
    <div v-if="!playing">
  <el-button type="primary"  v-if="hasConfig" v-on:click="loadConfig"> Load {{ configName }}</el-button>
...
<div v-if="showTime">
  ...
  <el-button type="primary" v-if="!paused" v-on:click="pause"> Pause Timer </el-button>
...
<div v-else>
  <div v-if="!playing">
    <row :gutter="20">
      <el-col :span="8">
        <el-input type="number" min=0 v-on:input="changeThis('hours', $event)" v-bind:value="hours">
          <template slot="append"> Hours </template>
        </el-input>
      </el-col>
      <el-col :span="8">

        <el-input type="number" min=0 v-on:input="changeThis('minutes', $event)" v-bind:value="minutes">
        <template slot="append"> Minutes </template>
        </el-input>
    </el-col>
    <el-col :span="8">

    <el-input type="number" min=0 v-on:input="changeThis('seconds', $event)" :value="seconds">
  ...
  <div v-else>
    ...
    <row class="center buttonRow">
      ...
      <el-button type="primary" v-if="playing" v-on:click="stopMusic"> Thanks, got it! </el-button>
    </row>
...
export default {
  ...
  updated: function () {
   if (this.$store.state.secondsLeft > 0 && this.$store.state.started && !this.$store.state.finished && !this.$store.state.paused && !this.$store.state.reset) {
     // set seconds left
      setTimeout(function(){this.$store.dispatch('decrement')}.bind(this), 1000);
    }
  ...
  methods: {
    ...mapActions( {'start': 'startTimer',  'pause': 'pauseTimer', 'decrement': 'decrement', 'resume': 'resumeTimer', 'reset': 'resetTimer', 'stopMusic': 'stopMusic', 'loadConfig': 'loadConfig'}),
    start: function() {
      ...
      if (min === 0 && sec === 0 && hours === 0) {
        ...
        this.$data.playErrorMessage = "Come on! You gotta work for at least a second before going on break!"
        setTimeout(function(){
      ...
      this.$store.dispatch('startTimer', { seconds: (hours*60*60) + (min*60) + sec*1});
        this.$data.minutes = 0;
        this.$data.seconds= 0;
        this.$data.hours = 0;
        this.$data.playError= false;
    ...
    loadConfig: function() {
      ...
      this.$data.minutes = Math.floor((this.$store.state.config.time % 3600 ) / 60)
      this.$data.seconds = Math.floor((this.$store.state.config.time % 3600) % 60)
      this.$store.dispatch("loadConfig");
    },
    changeThis: function(type, event){
      console.log(event);
      switch(type){
        case "minutes": this.$data.minutes = event; break;
        case "seconds": this.$data.seconds = event; break;
        case "hours": this.$data.hours = event; break;
        default: break;
      }
    }
  }
